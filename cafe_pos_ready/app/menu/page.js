...
export default function MenuPage() {
  ...
  async function updateGroup(id, name, sort) {
    await supabase.from('menu_groups').update({
      name,
      sort: Number(sort) || 0,
    }).eq('id', id);
    loadAll();
        ...
        <div style={{ display: 'grid', gap: 6, marginBottom: 10 }}>
          ...
          {groups.map((g) => (
            ...
            >
              ...
              <input
                value={g.name}
                onChange={(e) => updateGroup(g.id, e.target.value, g.sort)}
              />
              <input
                type="number"
                value={g.sort ?? 0}
                onChange={(e) => updateGroup(g.id, g.name, e.target.value)}
              />
        ...
        <div style={{ marginTop: 12, display: 'grid', gap: 6 }}>
          {itemsByGroup.map((it) => (
            <div
              ...
              style={{
                display: 'grid',
                gridTemplateColumns: '1fr 110px 80px 120px auto auto',
                gap: 8,
                alignItems: 'center',
              }}
            >
              ...
              <input
                type="number"
                value={it.sort ?? 0}
                onChange={(e) => updateItem(it.id, { sort: Number(e.target.value) || 0 })}
              />
              <select
